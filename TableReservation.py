...
while True:
    ...
    if not next_group or next_group - time.time() <= 0:
        ...
        next_group = time.time() + random.choice(flow_speed)
    # Update shop queues
    for shop in food_shops:
        ...
        if (
            ...
            shop["serving_end_time"] = time.time() + shop["serving_time"]
        elif (
            ...
            occupation["eating_duration"] = random.choice(shop["eating_time"])
